...
## Running the bot
You will need to setup a discord bot, a twitter application, and get the correct tokens from those setup as environment variables (refer to `src/main.py` for the environment variables needed). Set `ENABLE_AI=true` and `OPENAI_API_KEY` to enable AI replies. Tob detects OpenRouter (`sk-or-v1-`) and OrcaRouter (`sk-orca-`) keys and sends each provider's reasoning and web-search format. The example config uses OpenRouter with `openai/gpt-5-nano` and low reasoning; include `ultrathink` in a prompt to use high reasoning for that request. Add `--no-context` to an AI prompt to omit surrounding channel history for that request. Set `OPENAI_WEB_SEARCH=true` to allow Tob to browse the web.
